 ...
 *   stylestrap_. For example:
 *
 *     original, found in modules/field/field.module: theme_field()
 *     theme override, found in template.php: stylestrap_field()
 *
 *   where stylestrap is the name of your sub-theme. For example, the
...
function stylestrap_preprocess_maintenance_page(&$variables, $hook) {
  // When a variable is manipulated or added in preprocess_html or
  // preprocess_page, that same work is probably needed for the maintenance page
  // as well, so we can just re-use those functions to do that work here.
  stylestrap_preprocess_html($variables, $hook);
  stylestrap_preprocess_page($variables, $hook);
}
// */
 ...
 */
/* -- Delete this line if you want to use this function
function stylestrap_preprocess_html(&$variables, $hook) {
 ...
 */
function stylestrap_preprocess_page(&$variables, $hook) {
  global $user;
  ...
  //modal login block
  if (theme_get_setting('stylestrap_login_modal') && empty($user->uid)) {
    drupal_add_js(array('stylestrap_login_modal' => TRUE), 'setting');
    ...
    drupal_add_js(array('stylestrap_login_modal' => FALSE), 'setting');
    $variables['login_block'] = FALSE;
  }
  
  if (theme_get_setting('stylestrap_searchbox') && user_access('search content')) {
  ...
  if (($modal_form_paths = theme_get_setting('stylestrap_modal_forms'))) {
    $modal_forms = explode("\r\n", $modal_form_paths);
    drupal_add_js(array('stylestrap_forms_modal' => $modal_forms), 'setting');
  }
 ...
 */
function stylestrap_modal_form($element) {
    $form = $element['form'];
 ...
 */
function stylestrap_form_alter(&$form, &$form_state, $form_id) {
  if (isset($_GET['modal']) && $_GET['modal']) {
 ...
 */
function stylestrap_preprocess_block(&$variables) {
  $block = $variables['block'];
  //block groups as accordion
  static $acc_menu;
  if (!isset($acc_menu)) {
    $acc_menu = theme_get_setting('stylestrap_menu_accordion');
    if (!empty($acc_menu)) {
  ...
  $single_block = theme_get_setting('stylestrap_single_accordion');
 ...
 */
$path = drupal_get_path('theme', 'stylestrap');
$files = file_scan_directory($path . '/includes', '/.inc/');
foreach ($files as $absolute => $file) {
  require $absolute;
}

function stylestrap_preprocess_views_view_table(&$vars) {
  drupal_add_js(drupal_get_path('theme', 'stylestrap') . '/js/stylestrap_tableheader.js');
}
